---
title: "First Interaction with Quanto"
format: pdf
editor: visual
---
...
country_data %>%    
  ...
  arrange(life_expec) %>%    
  slice(1:10) %>%    
  ggplot(aes(x = reorder(country, -life_expec), 
             y =  life_expec)) +   
  geom_bar(stat = 'identity',            
           width = 0.4) +   
  theme_void() +   
  geom_col(fill = c("#E70F15")) +   
  geom_text(aes(label = life_expec), 
            size = 3,             
            fontface = 'bold', 
            hjust = 1.5,             
            color = 'white') +   
  ...
  labs(
    title = 'Which Countries have the shortest Life Expectancy',
